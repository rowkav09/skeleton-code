...
def DisplayErrorCodeInfo():
  ErrorMessages = {
    "1": "FileError: Unable to open file",
    "2": "FileReadError: Issue while reading file",
    "3": "SymbolError: Duplicate label",
    "4": "SyntaxError: Label missing ':'",
    "5": "OpcodeError: Invalid opcode",
    "6": "OperandError: Invalid operand",
    "7": "StateError: No source code loaded",
    "8": "StateError: Cannot edit before loading",
  ...
  if choice in ErrorMessages:
    print(ErrorMessages[choice])
  else:
    print("UnknownError: Code not recognised")
...
def AssemblerSimulator():
  ...
  while not Finished:
    DisplayMenu()
    MenuOption = (GetMenuOption()).upper()
    if MenuOption == 'L':
